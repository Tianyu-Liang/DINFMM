...
@everywhere using Bessels
@everywhere using MultiQuad

@everywhere const BOXES_PER_NODE = 4
@everywhere const DIMENSION = 2
@everywhere const PRECISION = ComplexF64
...
@everywhere function evaluate_diagonal(N)
    n = sqrt(N)
    h = 1 / n
    coefficient = 2 * n / 32 * pi
    #coefficient = 2 * n / 128 * pi

    func(x, y) = (0.25im * besselh(0, 1, coefficient * sqrt.(x.^2 + y.^2)));
    realpart(x, y) = real(func(x, y))
    impart(x, y) = imag(func(x, y))
 
    intgrl_r = 4 * dblquad(realpart, 0, h/2, 0, h/2)[1];
    intgrl_i = 4 * dblquad(impart, 0, h/2, 0, h/2)[1];
    intgrl = intgrl_r + intgrl_i * 1im;

    #intgrl = 4 * dblquad(func, 0, h/2, 0, h/2);

    return intgrl
end

# Defines the pointwise interaction kernel
@everywhere function form_kernel!(x1::AbstractArray{T, 2}, y1::AbstractArray{T, 2}, A::AbstractArray{ComplexF64, 2}) where T <: AbstractFloat
    ...
    temp = zeros(size(A))
    coefficient = 2 * sqrt(TOTAL_PTS) / 32 * pi
    ...
    #     end
    # end
    @inbounds @turbo for c in 1 : y_size, r in 1 : x_size
        temp[r, c] = coefficient * sqrt((x[1, r] - y[1, c])^2  + (x[2, r] - y[2, c])^2)
    end

    @inbounds for c in 1 : y_size, r in 1 : x_size
        A[r, c] = 0.25im * besselh(0, 1, temp[r, c])
    end
...
@everywhere function fft_kfun!(points::AbstractArray{T, 2}, idx::Union{AbstractArray{Int64, 1}, AbstractArray{T,}}, idy::Union{AbstractArray{Int64, 1}, AbstractArray{T,}}, diagonal::ComplexF64, A::AbstractArray{ComplexF64, 2}) where T<:Float64
...
@everywhere function kfun_internal!(points::AbstractArray{T, 2}, idx::Union{AbstractArray{Int64, 1}, AbstractArray{T,}}, idy::Union{AbstractArray{Int64, 1}, AbstractArray{T,}}, diagonal::ComplexF64, A::AbstractArray{ComplexF64, 2}) where T<:Float64
    @assert typeof(idx) <: AbstractRange || typeof(idx) <: Array{Int64, 1} || typeof(idx) <: Array{Float64,}
    @assert typeof(idy) <: AbstractRange || typeof(idy) <: Array{Int64, 1} || typeof(idy) <: Array{Float64,}
    N = size(points, 2)
    local_points = localpart(points)
    ...
    coefficient = 2 * sqrt(TOTAL_PTS) / 32 * pi
    Vx = zeros(size(sub_points_x, 2))
    @views Vx .= coefficient .* sqrt.(exp.(-32 .* ((sub_points_x[1, :] .- 0.5).^2 .+ (sub_points_x[2, :] .- 0.5).^2)));
    Vy = zeros(size(sub_points_y, 2))
    @views Vy .= coefficient .* sqrt.(exp.(-32 .* ((sub_points_y[1, :] .- 0.5).^2 .+ (sub_points_y[2, :] .- 0.5).^2)));
    ...
    #         #print(A[i, j], " ")
    #         #@printf("%f.3\n", A[i, j])
    #         @printf("%f.3  im%f.3, ", real(A[i, j]), imag(A[i, j]))
    #     end
    #     println()
    # end

    # increase diagonal by identity
    if !(typeof(idx) <: Array{Float64,}) && !(typeof(idy) <: Array{Float64,})
        @inbounds for c = 1 : length(idy)
            @inbounds for r = 1 : length(idx)
...
@everywhere function pfun_internal!(pxy_points::Array{T, 2}, index::Array{Int64, 1}, points::AbstractArray{T, 2}, A::Matrix{ComplexF64}) where T <: AbstractFloat
    ...
    coefficient = 2 * sqrt(TOTAL_PTS) / 32 * pi
    V = zeros(size(sub_points, 2))
    @views V .= coefficient .* sqrt.(exp.(-32 .* ((sub_points[1, :] .- 0.5).^2 .+ (sub_points[2, :] .- 0.5).^2)));
...
=#
@everywhere function fast_mul(points::AbstractArray{T, 2}, sqrtb::AbstractArray{T, 1}, diagonal::ComplexF64) where T <: AbstractFloat
    N = size(points, 2)
    n = Int64(sqrt(N))
    A = zeros(Complex{Float64}, N, 1)
    fft_kfun!(points, 1:N, [1], diagonal, A)
    ...
    fft!(B)
    #println(B)
    fft_mv(x) = fft_mv_(B, x, sqrtb)
    return fft_mv
end

@everywhere function fft_mv_(F::AbstractArray{T1, 2}, x::AbstractVector{T2} where T2 <: PRECISION, sqrtb::AbstractArray{Float64, 1} ) where T1 <: Number
    N = length(x)
    n = Int64(sqrt(N))
    larger = Matrix{Complex{Float64}}(undef, 2 * n - 1, 2 * n - 1)
    larger .= 0
    larger[1 : n, 1 : n] .= reshape(sqrtb .* x, n, n)
    fft!(larger)
    larger .= F .* larger
    ifft!(larger)
    y = sqrtb .* reshape(larger[1 : n, 1 : n], N, 1)
    return y + x
end
...
@everywhere function ldiv!(A::Function, x::AbstractVector)
...
@everywhere function rand_right(points::DArray)
    return rand(PRECISION, size(localpart(points), PART_TWO))
end
...
@everywhere function kernel_init(N::Int64, probes, use_MV::Bool=false)
    ...
    if use_MV == true
        ...
        coefficient = 2 * sqrt(TOTAL_PTS) / 32 * pi
        sqrtb = coefficient .* sqrt.(exp.(-32 .* ((default_points[1, :] .- 0.5).^2 .+ (default_points[2,:] .- 0.5) .^ 2)));
        MV = fast_mul(distribute(default_points, procs=[PART_ONE]), sqrtb, intgrl)
        array_parts = Array{Future, 1}(undef, length(probes))
        @sync for p in eachindex((probes))
...
@everywhere function permute_solution(factor_darray::DArray, v::DArray, rightside::DArray)
    ...
    ret_vec = zeros(PRECISION, num_points)
...
@everywhere function factorize_and_solve(N::Int64, layer::Int64, min_box_per_machine=4, worker_list=workers(); run_solve::Bool=false)
    ...
    GC.enable(true)
    println(size(modified_nodes[1][1].modified[1]))
    # Cholesky factor of matrix at top level
    fetch(@spawnat probes[1] x_matrix_nodes[1][1].X_RR = my_chol!(copy(modified_nodes[1][1].modified[1]))[1]);
    ...
    if multiply_verify == true
        let
            ...
            if use_MV
                ...
                #sol = solver!(probes, probe_num_progression, tree_parts, factor_nodes, communication_nodes, modified_nodes, x_matrix_nodes, rightside_nodes, recovered_point_permutation, generated_right, actual)
                #verify = MV(sol)
                #GG = zeros(ComplexF64, N, N)
                #kfun_internal!(distribute(default_assignment((1:2, 1:N), Int64(sqrt(N))), procs=[1]), 1:N, 1:N, evaluate_diagonal(N), GG)
                
                #println("norm: ", norm(verify - actual) / norm(actual))
        ...
        x_left = zeros(PRECISION, N)
        y_right = rand(PRECISION, N)
        y_right .= y_right ./ norm(y_right)
        info = gmres!(x_left, MV, y_right, reltol=1e-12, Pl=preconditioner_operator, log=true, maxiter=100)
