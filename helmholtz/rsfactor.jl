...
@everywhere function classify_points(points::DArray{T, 2}, worker_id::Int64, layer::Int64, max_layer::Int64, probes_length::Int64, box_length::AbstractFloat, min_point::Array{Float64, 1}) where {T <: AbstractFloat}
...
@everywhere function accumulate_update!(tree_darray::DArray, factor_darray::Array, communication_darray::DArray, coordinates::DArray{Float64, 2}, intgrl::PRECISION, determine_machine::Array, fetched_data::Array)
...
@everywhere function accumulate_update!(fetched_data::Array, factor_darray::Array, communication_darray::DArray, idx_range::Array{Int64, 1}, coordinates::DArray{Float64, 2}, intgrl::PRECISION)
    ...
    for candidate in eachindex(candidate_list)
        ...
        for i = 1 : length(smaller_list)
            ...
            if !isempty(up_idx)
                ...
                BLAS.gemm!('N', 'T', -1.0 + 0.0im, mat_a, mat_b, 1.0 + 0.0im, f_node_second.modified[idx])
                ...
                A = zeros(PRECISION, length(f_node.ske_index), length(other_f_node.ske_index))
                ...
                BLAS.gemm!('N', 'T', -1.0 + 0.0im, mat_a, mat_b, 1.0 + 0.0im, A)
                #A .-= change_matrix
...
@everywhere function my_chol!(A::AbstractMatrix)
...
@everywhere function perform_factorization!(tree_darray::DArray, factor_darray::Array, communication_darray::DArray, current_layer::Int64, kfun::Function,
    pfun::Function, on_boundary::Int64, coordinates::DArray{Float64, 2}, intgrl::PRECISION, store_size::DArray, fetched_map_future = Future[])
    ...
    T_len = (0, 0)
    A_SR_len = (0, 0)
    pre_allocate_mat = zeros(ComplexF64, 100, 500)
    proxy_mat = zeros(ComplexF64, 64, 100)
    ...
    @time for i in eachindex(factor_nodes)
        ...
        if current_layer > 1
            ...
            proxy_mat = zeros(ComplexF64, size(adjusted_circle, 2), length(f_node.ske_index))

            # if !(typeof(proxy_mat) <: Matrix)
            #     proxy_mat = transpose(proxy_mat)
            # end
            # proxy_mat = reshape(proxy_mat, length(proxy_mat))
            # resize!(proxy_mat, size(adjusted_circle, 2) * length(f_node.ske_index))
            # proxy_mat = reshape(proxy_mat, (size(adjusted_circle, 2), length(f_node.ske_index)))
        ...
        @views S_idx, R_idx, T = id(transpose(pre_allocate_mat[1 : length(f_node.ske_index), 1 : len_of_qr]), opts);
        ...
        #S_idx, R_idx, T = id(transpose(pre_allocate_mat), opts);
        ...
        #=
        _, R, p = qr(transpose(pre_allocate_mat[1 : length(f_node.ske_index), 1 : len_of_qr]), Val(true));
        ...
        pre_allocate_mat = transpose(pre_allocate_mat)
        ...
        T_ASR = transpose(T) * X_SR
        ASS_T = A_SS * T
        F = my_chol!(A_RR .- T_ASR .- transpose(T_ASR) .+ transpose(T) * ASS_T)
        upper_mat = Matrix(transpose(F[1]))
        #X_SR = (A_SR .- A_SS * T) / F.U
        BLAS.gemm!('N', 'N', -1.0 + 0.0im, A_SS, T, 1.0 + 0.0im, X_SR)
        BLAS.trsm!('R', 'U', 'N', 'N', 1.0 + 0.0im, upper_mat, X_SR)

        len_p = length(R_idx) + length(S_idx)

        X_NR = pre_allocate_mat[len_p + 1 : len_near, R_idx]
        BLAS.gemm!('N', 'N', -1.0 + 0.0im, pre_allocate_mat[len_p + 1 : len_near, S_idx], T, 1.0 + 0.0im, X_NR)
        BLAS.trsm!('R', 'U', 'N', 'N', 1.0 + 0.0im, upper_mat, X_NR)

        #X_NR = (submatrix[len_p + 1 : end, R_idx] - submatrix[len_p + 1 : end, S_idx] * T) / F.U;
        f_node_third.X_RR = F[1]
        f_node_third.X_SR = X_SR
        f_node_third.X_NR = X_NR
        s2 += (time() - t2)
        pre_allocate_mat = transpose(pre_allocate_mat) # revert it back so this array maintains horizontal
                ...
                if in_range(k1, idx_range)
                    ...
                    if !isempty(idx)
                        ...
                        if k1 == k2
                            # if k1 == k2, force matrix to be symmetric
                            if k1 == i + offset
                                @views BLAS.gemm!('N', 'T', -1.0 + 0.0im, X_SR, X_SR, 1.0 + 0.0im, k1_f_node_second.modified[idx])
                            else
                                @views BLAS.gemm!('N', 'T', -1.0 + 0.0im, X_NR[lenlist[k - 1] + 1 : lenlist[k], :], X_NR[lenlist[k - 1] + 1 : lenlist[k], :], 1.0 + 0.0im, k1_f_node_second.modified[idx])
                            end

                        else
                            if k1 == i + offset
                                @views BLAS.gemm!('N', 'T', -1.0 + 0.0im, X_SR, X_NR[lenlist[c - 1] + 1 : lenlist[c], :], 1.0 + 0.0im, k1_f_node_second.modified[idx])
                            elseif k2 == i + offset
                                @views BLAS.gemm!('N', 'T', -1.0 + 0.0im, X_NR[lenlist[k - 1] + 1 : lenlist[k], :], X_SR, 1.0 + 0.0im, k1_f_node_second.modified[idx])
                            else
                                @views BLAS.gemm!('N', 'T', -1.0 + 0.0im, X_NR[lenlist[k - 1] + 1 : lenlist[k], :], X_NR[lenlist[c - 1] + 1 : lenlist[c], :], 1.0 + 0.0im, k1_f_node_second.modified[idx])
                            end
                        end
                        ...
                        A = zeros(ComplexF64, length(k1_f_node.ske_index), length(k2_f_node.ske_index))
                        kfun(coordinates, mapped_points_k1, mapped_points_k2, intgrl, A)

                        if k1 == k2
                            # if k1 == k2, force matrix to be symmetric
                            if k1 == i + offset
                                @views BLAS.gemm!('N', 'T', -1.0 + 0.0im, X_SR, X_SR, 1.0 + 0.0im, A)
                            else
                                @views BLAS.gemm!('N', 'T', -1.0 + 0.0im, X_NR[lenlist[k - 1] + 1 : lenlist[k], :], X_NR[lenlist[k - 1] + 1 : lenlist[k], :], 1.0 + 0.0im, A)
                            end

                        else
                            if k1 == i + offset
                                @views BLAS.gemm!('N', 'T', -1.0 + 0.0im, X_SR, X_NR[lenlist[c - 1] + 1 : lenlist[c], :], 1.0 + 0.0im, A)
                            elseif k2 == i + offset
                                @views BLAS.gemm!('N', 'T', -1.0 + 0.0im, X_NR[lenlist[k - 1] + 1 : lenlist[k], :], X_SR, 1.0 + 0.0im, A)
                            else
                                @views BLAS.gemm!('N', 'T', -1.0 + 0.0im, X_NR[lenlist[k - 1] + 1 : lenlist[k], :], X_NR[lenlist[c - 1] + 1 : lenlist[c], :], 1.0 + 0.0im, A)
                            end
                        end
...
@everywhere function level_transition!(tree_darray::DArray, tree_below::DArray, factor_darray::Array, factor_level_below::Array, current_layer::Int64, kfun::Function, probe_list::Array{Int64, 1}, coordinates::DArray{Float64, 2}, intgrl::PRECISION,
...
@everywhere function optimize_level_transition!(factor_level_below::Array, prefetch::Future, total_length::Int64, match_p_idx::Int64,
    probe_list::Array{Int64, 1}, current_layer::Int64, idx_range::Array{Int64, 1}, coordinates::DArray{Float64, 2}, kfun::Function, intgrl::PRECISION)
...
@everywhere function build_matrix(factor_darray::Array, tree_nodes::Array{tree_node, 2}, communication_nodes::Array{communication_node, 2},
    proxy_mat::AbstractArray, nlist::Array{Int64, 1}, row_box::Int64,
        kfun::Function, offset::Int64, idx_range::Array{Int64, 1}, coordinates::DArray{Float64, 2}, intgrl::PRECISION,
    ...
    if !isempty(optional_mat)
        ...
        if size(optional_mat, 1) >= length(my_f_node.ske_index) && size(optional_mat, 2) >= colcount
            ...
            given_mat = zeros(ComplexF64, Int64(ceil(length(my_f_node.ske_index) * 1.1)), Int64(ceil(colcount * 1.5)))
        end
    else
        given_mat = zeros(ComplexF64, length(my_f_node.ske_index), colcount)
    end
    ...
    for c = 1 : length(nlist)
        ...
        if !isempty(idx)
            ...
            if smallerbox == row_box || !do_flip
                ...
                @views given_mat[1 : length(my_f_node.ske_index), colcount : colcount + length(colrange) - 1] .= transpose(f_node.modified[idx][colrange, rowrange]);
            end
...
@everywhere function forward_solve!(tree_darray::DArray, factor_darray::Array, communication_darray::DArray, v::DArray, current_layer::Int64, on_boundary::Int64; fetched_data=nothing)
    ...
    for i = 1 : length(factor_nodes)
        ...
        #------------------- Apply V^-1
        # apply U_t
        temp_r_idx = f_node_third.X_RR \ (v_node.right_side[r_idx] - transpose(f_node_third.T) * v_node.right_side[s_idx])
...
@everywhere function backward_solve!(tree_darray::DArray, factor_darray::Array, v::DArray, current_layer::Int64, on_boundary::Int64; fetched_data=nothing)
    ...
    for i = length(factor_nodes) : -1 : 1
        ...
        @views vec_ridx_1 = transpose(f_node_third.X_SR) * v_node.right_side[s_idx]
        # s2 += (time() - t1)
        vec_ridx_2 = zeros(ComplexF64, neighbor_idx[end]) # accumulate the vector, later used for X_RN multiplication
        ...
        # t1 = time()
        @views v_node.right_side[r_idx] .= transpose(f_node_third.X_RR) \ (v_node.right_side[r_idx] .- vec_ridx_1 .- transpose(f_node_third.X_NR) * vec_ridx_2)
...
@everywhere function solver!(probes::Array, probe_num_progression::Array, tree_parts::Array, factor_nodes::Array, communication_nodes::Array, modified_nodes::Array, 
    x_matrix_nodes::Array, rightside_nodes::Array, recovered_point_permutation::Vector, example_vec::DArray, given_vec::Union{DArray, AbstractVector})
    ...
    if typeof(given_vec) <: AbstractVector
...
@everywhere function middle_solve(rightside_nodes::Array, x_matrix_nodes::Array)
    rightside_nodes[1][1].right_side .= transpose(x_matrix_nodes[1][1].X_RR) \ (x_matrix_nodes[1][1].X_RR \ rightside_nodes[1][1].right_side)
end
